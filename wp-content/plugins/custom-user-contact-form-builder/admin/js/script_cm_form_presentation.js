...
formStyleApp.controller('formStyleCtrl', function ($scope, $http) {
    ...
    var elementIds = ["cm_style_container", "cm_textfield", "cm_btnfield","cm_field_label","cm_section_name"];
    $scope.selectForm = function () {
        $scope.selectedElement = 'cm_style_container';
        jQuery(".cm_style_action").css('display', 'block');
        jQuery(".cm_style_action").addClass('form_top_right');
        jQuery(".cm_style_action").removeClass('form_field_right btn_field_right');
    }
    $scope.selectTextField = function () {
        $scope.selectedElement = 'cm_textfield';
        jQuery(".cm_style_action").css('display', 'block');
        jQuery(".cm_style_action").addClass('form_field_right');
        jQuery(".cm_style_action").removeClass('form_top_right btn_field_rights');
    }
    $scope.selectButtonField = function () {
        $scope.selectedElement = 'cm_btnfield';
        jQuery(".cm_style_action").css('display', 'block');
        jQuery(".cm_style_action").addClass('btn_field_right');
        jQuery(".cm_style_action").removeClass('form_top_right form_field_right');
    }
    $scope.saveStyles = function () {
        /* Prapring json object */
        for (var i = 0; i < elementIds.length; i++) {
            if (elementIds[i] == "cm_style_container") {
                $scope.styles.style_form = jQuery("#" + elementIds[i]).attr('style');
            }
            if (elementIds[i] == "cm_textfield") {
                $scope.styles.style_textfield = jQuery("#" + elementIds[i]).attr('style');

            }
            if (elementIds[i] == "cm_btnfield") {
                $scope.styles.style_btnfield = jQuery("#" + elementIds[i]).attr('style');
                $scope.styles.form_submit_btn_label = jQuery("#" + elementIds[i]).val();
            }

            if(elementIds[i]=="cm_field_label"){
                $scope.styles.style_label = jQuery("#" + elementIds[i]).attr('style');
            }
            
            if(elementIds[i]=="cm_section_name"){
        ...
        $scope.styles.form_id = jQuery('#cm_form_id').val();
        $scope.styles.placeholder_css= jQuery("#cm_custom_style").html();
        

        $scope.styles.btn_hover_color= jQuery("#cm_btnfield").attr("data-btn-hover-color");
        $scope.styles.field_bg_focus_color= jQuery("#cm_textfield").attr("data-field-bg-focus-color");
        $scope.styles.text_focus_color= jQuery("#cm_textfield").attr("data-field-text-focus-color");


        
        $http({
            method: 'POST',
            url: ajaxurl + "?action=cm_save_form_view_sett",
            data: JSON.stringify($scope.styles),
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined}
        }).success(function (data) {
            if (data.toString().trim() === 'saved' || data.toString().trim() === 'not saved')
                window.location = '?page=cm_form_sett_manage&cm_form_id=' + $scope.styles.form_id;
...
formStyleApp.directive('styleActionBox', function (MediaUploader) {
    return {
        ...
        template: jQuery("#cm_styling_options").html(),
        link: function (scope, element, attributes, controller)
        {
            ...
            scope.executeAction = function () {
                ...
                if (selectedElement == "cm_style_container") {
                    // Check for Padding
                ...
                if (selectedElement == "cm_textfield") {
                    // Check for Label color
                    if (scope.styles.label_color)
                        jQuery("[id=cm_field_label]").css('color', "#" + scope.styles.label_color);
                    // Field Text color
                    ...
                    if (scope.styles.placeholder_color) {
                        ...
                        jQuery("#cm_custom_style").html(style);
                    }
            ...
            scope.close = function () {
                jQuery(".cm_style_action").css('display', 'none');
            }
            ...
            scope.$watch('styles.btn_hover_color', function (newValue, oldValue) {
                if (angular.isDefined(newValue)) {
                   jQuery("[id=" + selectedElement + "]").removeClass("cm_btn_focus"); 
                   oldValue=  jQuery("#" + selectedElement).css('background-color');
                    if (newValue != undefined) {
                        newValue= "#" + newValue;
                        if (selectedElement == "cm_btnfield") {
            ...
            scope.$watch('styles.text_focus_color', function (newValue, oldValue) {
                if (angular.isDefined(newValue)) {
                    ...
                    jQuery("[id=" + selectedElement + "]").removeClass("cm_field_focus_text");
                    newValue= "#" + newValue;
                    if (selectedElement == "cm_textfield") {
             ...
             scope.$watch('styles.field_bg_focus_color', function (newValue, oldValue) {
                if(angular.isDefined(newValue)){
                    jQuery("[id=" + selectedElement + "]").removeClass("cm_field_focus_bg");
