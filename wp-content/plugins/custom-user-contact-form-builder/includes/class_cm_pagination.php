...
class CM_Pagination {
    ...
    public function render($max_pages_without_abb=10,$max_visible_pages_near_current_page=3) {
        ...
        if ($this->total_pages > 1):
            $this->html .= '<ul class="cmpagination">';

            if ($this->curr_page > 1):
                $this->html .= '<li><a href="'.$this->query_string.'&cm_reqpage=1">' . CM_UI_Strings::get('LABEL_FIRST') . '</a></li>'.
                               '<li><a href="'.$this->query_string.'&cm_reqpage='.($this->curr_page-1).'">' . CM_UI_Strings::get('LABEL_PREVIOUS') . '</a></li>';
            endif;
            ...
            for ($i = $first_visible_page; $i <= $last_visible_page; $i++):
                if ($i != $this->curr_page):
                    $this->html .= '<li><a href="'.$this->query_string.'&cm_reqpage='.$i.'">'.$i.'</a></li>';
                else:
                    $this->html .= '<li><a class="active" href="'.$this->query_string.'&cm_reqpage=' . $i . '">' . $i . '</a></li>';
                endif;
            ...
            if ($this->curr_page < $this->total_pages):
                $this->html .= '<li><a href="'.$this->query_string.'&cm_reqpage='.$next_page.'">' . CM_UI_Strings::get('LABEL_NEXT') . '</a></li>' .
                        '<li><a href="'.$this->query_string.'&cm_reqpage='.$last_page.'">' . CM_UI_Strings::get('LABEL_LAST') . '</a></li>';
            endif;
