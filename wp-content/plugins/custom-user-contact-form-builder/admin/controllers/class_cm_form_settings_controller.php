...
/**
 * Description of class_cm_form_settings_controller
 *
 * @author Cmshelplive
 */
class CM_Form_Settings_Controller {
    ...
    function manage($form,  CM_Services $service, $request) {
        $data = new stdClass();
        if (!isset($request->req['cm_form_id']) || !(int)$request->req['cm_form_id'] || !$form->load_from_db($request->req['cm_form_id'])){
            $view = $this->mv_handler->setView('show_notice');
            $view->render(CM_UI_Strings::get('MSG_FS_NOT_AUTHORIZED'));
           return;
        }
                
        $form_options = $form->get_form_options();
        $global_option = new CM_Options;
        $acc_ctrl = $form_options->access_control;
        $data->form_id = (int)$request->req['cm_form_id'];
        $data->all_forms = CM_Utilities::get_forms_dropdown($service);
        unset($data->all_forms[$data->form_id]);
        $data->form = $form;
        $data->form_options = $form_options;
        $data->form_access = CM_UI_Strings::get('FD_LABEL_PUBLIC');
    ...
    function general($model, $service, $request, $params) {
        if ($this->mv_handler->validateForm("form_sett_general")) {
            $options = array();
            $options['form_type'] = CM_CONTACT_FORM;
            ...
            if (isset($request->req['cm_form_id']) && (int)$request->req['cm_form_id']) {
                $model->load_from_db($request->req['cm_form_id']);
                $model->set($options);
                $model->update_into_db();
                CM_Utilities::redirect('?page=cm_form_sett_manage&cm_form_id='.$request->req['cm_form_id']);
                return;
            } else {
                $model->set($options);
        ...
        $data = new stdClass();
        $data->autostart_tour = !CM_Utilities::has_taken_tour('form_gensett_tour');
    ...
    function limits($model, $service, $request, $params) {
        ...
        if ($this->mv_handler->validateForm("form_sett_limits")) {
            ...
            if (isset($request->req['cm_form_id']) && (int)$request->req['cm_form_id']) {
                ...
                echo '<div class="cmnotice">' . CM_UI_Strings::get('MSG_FS_NOT_AUTHORIZED') . '</div>';
                return;
            }
        }
        ...
        if (isset($request->req['cm_form_id']) && (int)$request->req['cm_form_id']) {
            ...
            $data->model = $model;
            $data->form_dropdown = CM_Utilities::get_forms_dropdown($service);
        } else {
            $view = $this->mv_handler->setView('show_notice');
            $data = CM_UI_Strings::get('MSG_FS_NOT_AUTHORIZED');
        }
        $view->render($data);
    }
    ...
    function view($model, $service, $request, $params) {
        if (!$request instanceof CM_Request) {
    ...
    function mailchimp($model, $service, $request, $params) {
        ...
        if (isset($request->req['cm_form_id']) && (int)$request->req['cm_form_id']) {
            $data = new stdClass();
            $data->form_id = $request->req['cm_form_id'];
            $model->load_from_db($request->req['cm_form_id']);
            ...
            $data->mailchimp_list[''] = CM_UI_Strings::get('SELECT_LIST');
 ...
 function ccontact($model, $service, $request, $params) {
    ...
    private function add_form_stats(&$data){
        ...
        $stat_service = new CM_Analytics_Service;
    ...
    private function add_form_submission_data(&$data){
        $service  = new CM_Submission_Service;
        ...
        foreach($latest_subs as $submission){
            ...
            /*if($user instanceof WP_User){
                ...
                $latest_sub_data[$i]->user_avatar = get_avatar_url($user->ID) ? : CM_IMG_URL.'placeholder.jpg';
            }
            else*/{
                $latest_sub_data[$i]->user_name = $submission->user_email;
                $latest_sub_data[$i]->user_avatar = get_avatar_url($submission->user_email) ? : CM_IMG_URL.'placeholder.jpg';
            }
            $latest_sub_data[$i]->is_read = $submission->is_read;
            $latest_sub_data[$i]->submitted_on = CM_Utilities::localize_time($submission->submitted_on);
    ...
    private function add_form_attchment_data(&$data){
        $service = new CM_Attachment_Service();
            ...
            foreach ($attachments as $att) {
                ...
                switch ($att_type) {
                    case 'image':
                        ...
                        break;
                    case 'default':
                        $data->attachments[$i]->image_url = CM_IMG_URL . 'attachment-placeholder.png';
                }
                $data->attachments[$i]->name = basename($att->guid);
                $data->attachments[$i]->date = CM_Utilities::localize_time($att->post_date);
                $data->attachments[$i]->is_new = $att->post_date >= CM_Utilities::get_current_time(time() - (60 * 60));
                $i++;
            }
    }
    ...
    private function add_form_timewise_stat(&$data){
        ...
        $service = new CM_Analytics_Service();
