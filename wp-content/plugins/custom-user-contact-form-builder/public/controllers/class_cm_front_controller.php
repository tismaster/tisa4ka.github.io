...
/**
 * Description of class_cm_front_controller
 *
 * @author CMSHelplive
 */
class CM_Front_Controller {
    ...
    public function set_otp($model, $service, $request, $params) {
        ...
        if (isset($request->req['cm_otp_email']))
            $email = $request->req['cm_otp_email'];

        if (isset($request->req['cm_otp_key']))
            $key = $request->req['cm_otp_key'];

        if (isset($request->req['cm_user_key'], $request->req['cm_username'])) {
            $user_key = $request->req['cm_user_key'];
            $username = $request->req['cm_username'];
            $remember = isset($request->req['cm_remember']) ? true : false;
    ...
    public function submissions($model, CM_Front_Service $service, $request, $params) {
        ...
        if (null != $user_email) {
            ...
            if (isset($request->req['submission_id'])) {
                ...
                $msg = CM_UI_Strings::get('MSG_INVALID_SUBMISSION_ID_FOR_EMAIL');
                ...
                $req_page_sub = (isset($request->req['cm_reqpage_sub']) && $request->req['cm_reqpage_sub'] > 0) ? $request->req['cm_reqpage_sub'] : 1;
                $offset_sub = ($req_page_sub - 1) * $entries_per_page_sub;

                if (isset($request->req['cm_edit_user_details'])) {
                ...
                if ($submissions) {
                    ...
                    $req_page_pay = (isset($request->req['cm_reqpage_pay']) && $request->req['cm_reqpage_pay'] > 0) ? $request->req['cm_reqpage_pay'] : 1;
                    ...
                    $data->active_tab_index = $distinct ? 1 : (isset($request->req['cm_tab']) ? (int) $request->req['cm_tab'] : 0);
                    ...
                    $view = $this->mv_handler->setView('not_authorized', true);
                    $msg = CM_UI_Strings::get('MSG_NO_SUBMISSION_FRONT');
                    return $view->read($msg);
                }
            }
        } else {
            ...
            $view = $this->mv_handler->setView('not_authorized', true);
            $msg = CM_UI_Strings::get('MSG_NOT_AUTHORIZED');
            return $view->read($msg);
        }
    }

    public function log_off($model, CM_Front_Service $service, $request, $params) {
        $user_email = $service->get_user_email();

        if (null != $user_email) {
            $service->log_front_user_off($user_email);
            if (!isset($request->req['cm_do_not_redirect']))
                CM_Utilities::redirect(get_permalink(get_option('cm_option_front_sub_page_id')));
            else
                CM_Utilities::redirect(get_permalink());
        }
    }
