 ...
 * @author CMSHelplive
 */
class CM_Analytics_Service extends CM_Services
{
  ...
  {
  	return CM_DBManager::get('STATS', array('form_id' => $form_id), array('%d'), 'results', $offset, $limit, '*', 'visited_on', true);
  }
  
  public function get_average_filling_time($form_id)
  {
    $avg = CM_DBManager::get_average_value('STATS','time_taken', array('form_id' => $form_id));
  ...
  {
    //CM_DBManager::delete_and_reset_table('STATS');
    CM_DBManager::delete_rows('STATS', array('form_id'=>$form_id), array('%d'));
  }

  public function get_field_stats($form_id)
  {
    $fields = CM_DBManager::get_fields_filtered_by_types($form_id, array('Select','Checkbox','Radio','Country'));
    $stat_arr = array();
    
    $subf_table = CM_Table_Tech::get_table_name_for('SUBMISSION_FIELDS');
    $subs_table = CM_Table_Tech::get_table_name_for('SUBMISSIONS');
    ...
    foreach($fields as $field)
    {
      ...
      {
         case 'Checkbox':
             $temp = CM_DBManager::run_query("SELECT value FROM `$subf_table` sf, `$subs_table` ss WHERE  `field_id` = $field->field_id AND sf.submission_id = ss.submission_id AND ss.child_id = 0 ORDER BY `sub_field_id`",'col');
            ...
            foreach($vals as $val)
             ...
             $tmp_obj->total_sub = (int)CM_DBManager::run_query("SELECT COUNT(sub_field_id) FROM `$subf_table` sf, `$subs_table` ss WHERE `field_id` = $field->field_id AND sf.submission_id = ss.submission_id AND ss.child_id = 0",'var');
             
           break;

        case 'Country':
            $temp = CM_DBManager::run_query("SELECT `value`, COUNT(*) AS `count` FROM `$subf_table` sf, `$subs_table` ss WHERE `field_id` = $field->field_id AND sf.submission_id = ss.submission_id AND ss.child_id = 0 GROUP BY `value`");
  ...
  public function get_browser_usage($form_id)
  {
      ...
      $temp = CM_DBManager::count_multiple('STATS', "browser_name", array('form_id' => $form_id), array('browser_name' => 'Chrome,Edge,Safari,Opera,Internet Explorer,Firefox,iPhone,Android,BlackBerry'));
      ...
      $temp = CM_DBManager::count_multiple('STATS', "browser_name", array('form_id' => $form_id, 'submitted_on' => 'not null', 'submitted_on' => '!banned'), array('browser_name' => 'Chrome,Edge,Safari,Opera,Internet Explorer,Firefox,iPhone,Android,BlackBerry'));
...
//echo "<pre>",var_dump($browsers),"</pre>";
  ...
  {        
        ...
        $visits = CM_DBManager::get_generic('STATS', $sel_c, $wh_c, OBJECT_K);
        ...
        $submissions = CM_DBManager::get_generic('STATS', $sel_c, $wh_c, OBJECT_K);      
  ...
  public function get_visitors_count($form_id){
      return CM_DBManager::get_visitors_count($form_id);
  }
