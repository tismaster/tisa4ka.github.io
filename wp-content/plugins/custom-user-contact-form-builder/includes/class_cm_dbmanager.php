...
class CM_DBManager
{
    ...
    {
        if (!isset(self::$instance) && !( self::$instance instanceof CM_DBManager ))
        {
            self::$instance = new CM_DBManager;
        }
    ...
    public static function insert_row($model_identifier, $array_attributes, $array_attribute_format)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for($model_identifier);
    ...
    public static function update_row($model_identifier, $unique_id_value, $array_attributes, $array_attribute_format)
    {
        ...
        $unique_id_name = CM_Table_Tech::get_unique_id_name($model_identifier);
    ...
    public static function remove_fields_for_page($page_no, $form_id)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for('FIELDS');
    ...
    public static function get_fields_by_form_id($form_id)
    {
        ...
        $foreign_key = CM_Table_Tech::get_unique_id_name('FORMS');
    ...
    public static function set_field_order($order_list)
    {
        ...
        $unique_id_name = CM_Table_Tech::get_unique_id_name('FIELDS');
    ...
    public static function get_submissions_for_form($form_id, $limit = 9999999, $offset = 0, $column = '*', $sort_by = '', $descending = false)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for('SUBMISSIONS');

        if (empty($sort_by))
        {

            $unique_id_name = CM_Table_Tech::get_unique_id_name('SUBMISSIONS');
    ...
    public static function get_fields_for_submission($submission_id)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for('SUBMISSION_FIELDS');

        $unique_id_name = CM_Table_Tech::get_unique_id_name('SUBMISSION_FIELDS');
    ...
    public static function search_submissions_for($field_id, $field_value, $limit = 9999999, $offset = 0, $sort_by = '', $descending = false)
    {
        ...
        $foreign_key['submission'] = CM_Table_Tech::get_unique_id_name('SUBMISSIONS');

        $foreign_key['field'] = CM_Table_Tech::get_unique_id_name('FIELDS');
 ...
 public static function group_by_total($model_identifier, $where, $data_specifiers = '',$group_by=null)
    ...
    public static function get_generic($identifier, $select_clause, $where_clause, $format = OBJECT)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for($identifier);

        $unique_id_name = CM_Table_Tech::get_unique_id_name($identifier);
    ...
    public static function delete_form_submissions($form_id)
    {
        global $wpdb;
        $table_name = CM_Table_Tech::get_table_name_for('SUBMISSIONS');
        $table_name_ = CM_Table_Tech::get_table_name_for('SUBMISSION_FIELDS');
        $foreign_key = CM_Table_Tech::get_unique_id_name('FORMS');
        if ($foreign_key === false)
            return false;
        $result = $wpdb->query("DELETE FROM `$table_name` where `$foreign_key` = $form_id");
    ...
    {
        global $wpdb;
        $table_name = CM_Table_Tech::get_table_name_for('PAYPAL_LOGS');
        $foreign_key = CM_Table_Tech::get_unique_id_name('FORMS');
        if ($foreign_key === false)
            return false;
        $result = $wpdb->query("DELETE FROM `$table_name` where `$foreign_key` = $form_id");
    ...
    {
        global $wpdb;
        $table_name = CM_Table_Tech::get_table_name_for('STATS');
        $foreign_key = CM_Table_Tech::get_unique_id_name('FORMS');
        if ($foreign_key === false)
            return false;
        $result = $wpdb->query("DELETE FROM `$table_name` where `$foreign_key` = $form_id");
    ...
    {
        global $wpdb;
        $table_name = CM_Table_Tech::get_table_name_for('NOTES');
        $foreign_key = CM_Table_Tech::get_unique_id_name('FORMS');
        $foreign_key_sub = CM_Table_Tech::get_unique_id_name('SUBMISSIONS');
    ...
    {
        global $wpdb;
        $table_name = CM_Table_Tech::get_table_name_for('FORMS');
        $primary_key = CM_Table_Tech::get_unique_id_name('FORMS');
    ...
    public static function get_results_for_last_col($interval, $form_id, $field_id, $field_value, $offset = 0, $limit = 999999, $sort_by = 'submission_id', $descending = false)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for('SUBMISSIONS');
        $col_name = CM_Table_Tech::get_unique_id_name('SUBMISSIONS');
    ...
    public static function sidebar_user_search($criterion, $type)
    {
        ...
        if ($type == "user_status")
        {
            ...
            if (count($criterion) > 1)
                ...
                $query = "Select distinct user_id from $table_name where meta_key='cm_user_status' and meta_value=" . $criterion[0];
            $result = $wpdb->get_results($query);
            foreach ($result as $el)
            {
                $user_ids[] = $el->user_id;
    ...
    public static function delete_front_user($interval, $time_format, $by_last_activity = false)
    {
        ...
        $table_name = CM_Table_Tech::get_table_name_for('FRONT_USERS');

        if ($by_last_activity)
            $qry = "DELETE FROM $table_name WHERE `last_activity_time` < '" . CM_Utilities::get_current_time(time() - $interval * $mul) . "'";
        else
            $qry = "DELETE FROM $table_name WHERE `created_date` < '" . CM_Utilities::get_current_time(time() - $interval * $mul) . "'";
    ...
    public static function update_last_activity()
    {
        ...
        return $wpdb->query("UPDATE $table_name set `last_activity_time`= '" . CM_Utilities::get_current_time() . "'");
    }
    ...
    {
        CM_Table_Tech::delete_and_reset_table($identifier);
    }
    ...
    {
        /* SELECT `value`, COUNT(*) FROM `wp_cm_submission_fields` WHERE `field_id` = 94 GROUP BY `value` */
        global $wpdb;
    ...
    public static function get_submissions($form_id=0,$filter,$selection="*",$sort_by = 'submission_id', $descending = true){
        ...
        if (isset($filters['cm_field_to_search']) && (int) $filters['cm_field_to_search'])
        {
            $sub_ids = self::search_submissions_for($filters['cm_field_to_search'], $filters['cm_value_to_serach'], 999999, 0, null, false);
            if ($sub_ids)
                $sub_ids = implode(',', $sub_ids);
            $searched = true;
        }

        switch (strtolower($filters['cm_interval']))
        {
            case 'today':
                $interval_string = 'BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 day)';
                break;
            ...
            case 'custom':
                   ...
                   if($filters['cm_fromdate'] != '' || $filters['cm_dateupto'] != '')
                   {
                       if($filters['cm_fromdate'] == '')
                       {
                           $interval_string = '<= \''.$filters['cm_dateupto'].'\'';
                       }
                       elseif($filters['cm_dateupto'] == '')
                       {
                           $interval_string = '>= \''.$filters['cm_dateupto'].'\'';
                       }
                       else
                       $interval_string = 'BETWEEN \''.$filters['cm_fromdate'].' 00:00:00\' AND \''.$filters['cm_dateupto'].' 23:59:59\'';
                       
                       break;
                   }
                   //Let it fall through to 'all' case.
            case 'all':
                if ((int) $filters['cm_field_to_search'] && $sub_ids)
    ...
    {
          global $wpdb;
          $table_name = CM_Table_Tech::get_table_name_for('SENT_EMAILS');
          $uid = CM_Table_Tech::get_unique_id_name('SENT_EMAILS');
          $qry = "";
          ...
          if (isset($filters['cm_field_to_search'],$filters['cm_value_to_search']) && trim($filters['cm_value_to_search']) != '')
          {
              $searched = true;
              //Sanitize against incorrect column name
              if(!in_array($filters['cm_field_to_search'], array('to','sub','body')))
                      $filters['cm_field_to_search'] = 'body';

              $field_name = $filters['cm_field_to_search'];
              ...
              $search_term = trim($filters['cm_value_to_search']);
