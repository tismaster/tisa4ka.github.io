 ...
 * @subpackage gorilla_forms/admin/models
 */
 ...
 * @author cmshelplive
 */
class CM_Forms extends CM_Base_Model {
    ...
    public $form_options;
    public $errors;
    private $form_builder_id = "cm_form_add";
    ...
    public function set_form_name($form_name) {
        if ($form_name)
            ...
            CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_DATE_INVALID'));
    }
    ...
    public function set_form_options($options) {
        ...
        $this->form_options = CM_Utilities::merge_object($form_options, $this->form_options);
    }
    ...
    public function validate_autoresponder($form_builder_id) {
        ...
        if ($this->get_form_should_send_email() == "1") {
            if ($this->form_options->form_email_content == "") {
                CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_AUTO_REPLY_CONTENT_INVALID'));
                $valid = false;
            }

            if ($this->form_options->form_email_subject == "") {
                CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_AUTO_REPLY_SUBJECT_INVALID'));
                $valid = false;
            }
        }
        return $valid;
        ...
        if ($this->form_redirect == "page" || $this->form_redirect == "url") {
            ...
            switch ($this->form_redirect) {
                case "page": if (empty($this->form_redirect_to_page)) {
                        CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_REDIRECT_PAGE_INVALID'));
                        $valid = false;
                    }
                    break;
                case "url": if (empty($this->form_redirect_to_url)) {
                        CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_REDIRECT_URL_INVALID'));
                        $valid = false;
                    }
                    break;
            }
            ...
            if (isset($this->form_options->form_expired_by) && !empty($this->form_options->form_expired_by)) {
                ...
                switch ($this->form_options->form_expired_by) {
                    ...
                    case "submissions": if (empty($this->form_options->form_submissions_limit)) {
                            CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_LIMIT_INVALID'));
                            $valid = false;
                        } break;

                    case "date": if (empty($this->form_options->form_expiry_date)) {
                            CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_DATE_INVALID'));
                            $valid = false;
                        }
                        break;
                    case "both": if (empty($this->form_options->form_expiry_date) || empty($this->form_options->form_submissions_limit)) {
                            CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_BOTH_INVALID'));
                            $valid = false;
                        }
                        break;
                }
                ...
                $valid = false;
                CM_PFBC_Form::setError($form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_INVALID'));
            }
        }
        return $valid;
    }
...
//    }
    ...
    public function insert_into_db() {
        ...
        $result = CM_DBManager::insert_row('FORMS', $data, $data_specifiers);
    ...
    public function update_into_db() {
        ...
        $result = CM_DBManager::update_row('FORMS', $this->form_id, $data, $data_specifiers);
    ...
    public function update_form_options($form_id, array $data, array $data_specifiers) {
        ...
        $form_options_ex = CM_DBManager::get('FORMS', array('form_id' => $form_id), array('%d'), 'var', 0, 1, 'form_options');

        $form_options_ex = maybe_unserialize($form_options_ex);

        $form_options_ex = CM_Utilities::merge_object($form_options_ex, $this->form_options);

        $form_options = CM_Utilities::merge_object($form_options, $form_options_ex);
        ...
        //$result = CM_DBManager::update_row('FORMS', $form_id, $row_data, $row_data_spec);
    ...
    public function load_from_db($form_id, $should_set_id = true) {
        ...
        $result = CM_DBManager::get_row('FORMS', $form_id);
    ...
    public function remove_from_db() {
        return CM_DBManager::remove_row('FORMS', $this->form_id);
    }
    ...
    public function validate_model() {
        ...
        if ($this->form_redirect == "page" || $this->form_redirect == "url") {
            ...
            switch ($this->form_redirect) {
                case "page": if (empty($this->form_redirect_to_page)) {
                        CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_REDIRECT_PAGE_INVALID'));
                        $valid = false;
                    }
                    break;
                case "url": if (empty($this->form_redirect_to_url)) {
                        CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_REDIRECT_URL_INVALID'));
                        $valid = false;
                    }
                    break;
            }
        ...
        if ($this->form_should_auto_expire) {
            if (isset($this->form_options->form_expired_by) && !empty($this->form_options->form_expired_by)) {
                ...
                switch ($this->form_options->form_expired_by) {
                    ...
                    case "submissions": if (empty($this->form_options->form_submissions_limit)) {
                            CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_LIMIT_INVALID'));
                            $valid = false;
                        } break;
                    ...
                    case "date": if (empty($this->form_options->form_expiry_date)) {
                        ...
                        break;
                    case "both": if (empty($this->form_options->form_expiry_date) || empty($this->form_options->form_submissions_limit)) {
                            CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_BOTH_INVALID'));
                            $valid = false;
                        }
                        break;
                }
                ...
                $valid = false;
                CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_EXPIRY_INVALID'));
            }
        }
        ...
        if ($this->get_form_should_send_email() == "1") {
            if ($this->form_options->form_email_content == "") {
                CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_AUTO_REPLY_CONTENT_INVALID'));
                $valid = false;
            }

            if ($this->form_options->form_email_subject == "") {
                CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MSG_AUTO_REPLY_SUBJECT_INVALID'));
                $valid = false;
            }
        }
         ...
         */
        /*
          if(get_option('cm_option_enable_mailchimp')=="yes" ) {

          if (isset($this->form_options->mailchimp_list) && $this->form_options->mailchimp_list == 0) {
          $valid = false;
          CM_PFBC_Form::setError($this->form_builder_id, CM_UI_Strings::get('MAILCHIMP_LIST_ERROR'));
          }
          }
         */
        /*
