...
class CM_Paypal_Service implements CM_Gateway_Service
{
    private $paypal;
    private $options;
    ...
    function __construct() {
        ...
        $this->paypal = new cm_paypal_class();
    ...
    public function callback($payment_status,$cm_pproc_id)
    {
        switch ($payment_status)
            {
                case 'success':
                    ...
                    {
                        $log_id = $cm_pproc_id;
                        $log = CM_DBManager::get_row('PAYPAL_LOGS', $log_id);
                        if ($log)
                        {
                            if ($log->log)
                            {
                                ...
                                {
                                    $ffact = new CM_Form_Factory();
                                    $fef = $ffact->create_form($form_id);
                                    $fopt = $fef->get_form_options();
                                    if($fopt->auto_login)
                                         $_SESSION['CM_SLI_UID'] = $user_id;
                                    
                                    echo '<div id="cmform">';
                                    echo "<div class='cminfotextfront'>" . CM_UI_Strings::get("MSG_PAYMENT_SUCCESS") . "</br>";
                                    echo '</div></div>';
                                    return 'success';
                                ...
                                {
                                    echo '<div id="cmform">';
                                    echo "<div class='cminfotextfront'>" . CM_UI_Strings::get("MSG_PAYMENT_FAILED") . "</br>";
                                    echo '</div></div>';
                                    return 'failed';
                                } else if ($payment_status == 'In-Progress' || $payment_status == 'Pending' || $payment_status == 'Processed')
                                {
                                    echo '<div id="cmform">';
                                    echo "<div class='cminfotextfront'>" . CM_UI_Strings::get("MSG_PAYMENT_PENDING") . "</br>";
                                    echo '</div></div>';
                                    return 'pending';
                ...
                case 'cancel':
                    echo '<div id="cmform">';
                    echo "<div class='cminfotextfront'>" . CM_UI_Strings::get("MSG_PAYMENT_CANCEL") . "</br>";
                    echo '</div></div>';
                    return;
                ...
                case 'ipn':
                    ...
                    $curr_date = CM_Utilities::get_current_time(); // date_i18n(get_option('date_format'));

                    CM_DBManager::update_row('PAYPAL_LOGS', $log_entry_id, array(
    ...
    public function charge($data,$pricing_details) {
        ...
        $form_no = $cm_form_diary[$form_id];        
        
        if(false == $this_script){
            $this_script = admin_url('admin-ajax.php?action=contactmagic_embedform&form_id='.$data->form_id);
        }
        ...
        $this->paypal->add_field('return', $this_script . $sign . 'cm_pproc=success&cm_pproc_id=0'.'&cm_fid='.$form_id.'&cm_fno='.$form_no); // return URL after the transaction got over
        $this->paypal->add_field('cancel_return', $this_script . $sign . 'cm_pproc=cancel&cm_pproc_id=0'.'&cm_fid='.$form_id.'&cm_fno='.$form_no); // cancel URL if the trasaction was cancelled during half of the transaction
        $this->paypal->add_field('notify_url', $this_script . $sign . 'cm_pproc=ipn&cm_pproc_id=0'.'&cm_fid='.$form_id.'&cm_fno='.$form_no); // Notify URL which received IPN (Instant Payment Notification)
        ...
        if ($total_amount <= 0.0)
        {
            $log_entry_id = CM_DBManager::insert_row('PAYPAL_LOGS', array('submission_id' => $data->submission_id,
                        'form_id' => $form_id,
                        'invoice' => $invoice,
                        'status' => 'Completed',
        ...
        $this->paypal->add_field('return', $this_script . $sign . 'cm_pproc=success&cm_pproc_id='.$log_entry_id.'&cm_fid='.$form_id.'&cm_fno='.$form_no); // return URL after the transaction got over
        $this->paypal->add_field('cancel_return', $this_script . $sign . 'cm_pproc=cancel&cm_pproc_id='.$log_entry_id.'&cm_fid='.$form_id.'&cm_fno='.$form_no); // cancel URL if the trasaction was cancelled during half of the transaction
        $this->paypal->add_field('notify_url', $this_script . $sign . 'cm_pproc=ipn&cm_pproc_id='.$log_entry_id.'&cm_fid='.$form_id.'&cm_fno='.$form_no); // Notify URL which received IPN (Instant Payment Notification)
