...
class CM_Migrator
{
    ...
    {
        $status = get_site_option('cm_mig_log', '<br>');
        error_log("in: " . $status);

        update_site_option('cm_mig_log', $status);
            ...
            if ($dst_name)
            {
                if ($dst_name === 'smtp_password')
                {
                    ...
                    if (!$oldp)
                    {
                        ...
                        update_option("cm_option_" . $dst_name, $newp);
                    }
                }
                update_option("cm_option_" . $dst_name, get_option($src_name, null));
...
// die('here');
        ...
        global $wpdb;
        $crf_option = $wpdb->prefix . "cfp_option";
...
//return;
        ...
        foreach ($data as $row)
        {
            ...
            {
                if ($row->fieldname === 'cfp_theme')
                {
                    if ($row->value === 'default')
                        update_option("cm_option_theme", 'matchmytheme');
                    else
                        update_option("cm_option_theme", 'classic');
                } else
                    update_option("cm_option_" . $data_mapping[$row->fieldname], $row->value);
        ...
        $x = get_option('cfp_smtp_from_email_name', null);
        if ($x)
        {
            update_option("cm_option_senders_display_name", $x);
        }

        $x = get_option('cfp_smtp_from_email_address', null);
        if ($x)
        {
            update_option("cm_option_senders_email", $x);
        }

        $x = get_option('cfp_smtp_autentication', null);
        if ($x)
        {
            if ($x === 'true')
                update_option("cm_option_smtp_auth", 'yes');
            else
                update_option("cm_option_smtp_auth", 'no');
        }

//        $x = get_option('cfp_theme', null);
//        if ($x)
//        {
//            if ($x === 'default')
//                update_option("cm_option_theme", 'matchmytheme');
...
//        }
        ...
        $x = get_option('cfp_smtp_encription', null);

        switch ($x)
        {
            case 'tls':
                update_option("cm_option_smtp_encryption_type", 'enc_tls');
                break;

            case 'ssl':
                update_option("cm_option_smtp_encryption_type", 'enc_ssl');
                break;

            default:
                update_option("cm_option_smtp_encryption_type", 'enc_none');
                break;
        }
    }
...
//$this->migrate('paypal_logs', 'crf_paypal_logs', 'cm_paypal_logs');

        $res = $wpdb->get_results("SELECT * FROM `" . $wpfx . "cfp_stats` WHERE 1 LIMIT $offset, $limit", ARRAY_A);
...
//var_dump($details['submit_time']);echo"<br>ONErun<br><br>";
            ...
            {
                $browser = new CM_Browser($details['Browser']);
...
//echo "<pre>", var_dump($res), "</pre>";
        $this->insert_array('stats', $wpfx . "cm_stats", $res);
    }
    ...
    public function migrate_forms()
    {
        ...
        $table_name_src = $wpdb->prefix . 'cfp_forms';
        $table_name_dst = $wpdb->prefix . 'cm_forms';
        $table_name_fields = $wpdb->prefix . 'cfp_fields';
    ...
    public function migrate_fields($offset = 0, $limit = 99999999)
    {
        ...
        $table_name_src = $wpdb->prefix . 'cfp_fields';
        $table_name_dst = $wpdb->prefix . 'cm_fields';
...
//$sql = esc_sql($sql);
    ...
    public function migrate_submissions($offset = 0, $limit = 9999999)
    {
        ...
        $table_name_fields = $wpdb->prefix . 'cfp_fields';
        $table_name_pfields = $wpdb->prefix . 'cfp_fields';
        $table_name_plogs = $wpdb->prefix . 'cfp_paypal_log';
        $table_name_src = $wpdb->prefix . 'cfp_submissions';
        $table_name_dst = $wpdb->prefix . 'cm_submission_fields';
        $table_name_dst_sub = $wpdb->prefix . 'cm_submissions';
        $table_name_fields_rm = $wpdb->prefix . 'cm_fields';

        $options = new CM_Options;

        $srcdata = $wpdb->get_results("SELECT * FROM $table_name_src WHERE 1 LIMIT $offset, $limit");
        $fieldsdata = $wpdb->get_results("SELECT * FROM $table_name_fields WHERE 1");
        $pfieldsdata = null;//$wpdb->get_results("SELECT * FROM $table_name_pfields WHERE 1");
        $plogsdata = null;//$wpdb->get_results("SELECT * FROM $table_name_plogs WHERE 1");
                        ...
                        if ($a === 'field')
                        {
                            switch ($s->$a)
                            {
                                ...
                                case 'token' :
                                    ...
                                    break;
                                case 'entry_time' :
                                    $dst_data_sub[$s->submission_id]['submitted_on'] = CM_Utilities::get_current_time($s->value);
                                    $is_row_valid = false;
                                    break;
                                case 'user_email' :
                ...
                if (count($users) !== 0)
                {
                    foreach ($users as $user)
                    {
                        if ($user['form_type'] === 'reg_form' && ((isset($user['user_approval']) && $user['user_approval'] != 'yes') || (isset($user['payment_status']) && $user['payment_status'] === 'pending')) && isset($user['user_email']) && is_email($user['user_email']) && !email_exists($user['user_email']) && isset($user['user_pass']) && isset($user['user_name']))
                        {
                            ...
                            update_user_meta($user_id, 'cm_user_status', 1);
...
//error_log("Query: ".$qry);
    ...
    {
        $user_query = new WP_User_Query(array('meta_key' => 'cfp_user_status', 'meta_value' => 'deactivate', 'fields' => 'ID'));
...
//Mappers.
    private function get_options_mapping()
    {
        return array('ucf_enable_cpatcha_login' => 'enable_captcha',
            'cfp_recaptcha_lang' => null,
            'cfp_recaptcha_request_method' => null,
            'cfp_note_notification' => 'user_notification_for_notes',
            'cfp_f_sub_page_id' => 'front_sub_page_id',
            'cfp_migrate_submission' => null,
            ...
            'cfp_whatsnewpage' => null,
            'cfp_gateway' => 'payment_gateway',
            'cfp_test_mode' => 'paypal_test_mode',
            'cfp_paypal_email' => 'paypal_email',
            'cfp_currency' => 'currency',
            'cfp_paypal_page_style' => 'paypal_page_style',
            'cfp_currency_position' => 'currency_symbol_position',
            ...
            'cfp_smtp' => 'enable_smtp',
            'cfp_smtp_host' => 'smtp_host',
            'cfp_smtp_encription' => 'smtp_encryption_type',
            'cfp_smtp_port' => 'smtp_port',
            'cfp_smtp_autentication' => 'smtp_auth',
            'cfp_smtp_username' => 'smtp_user_name',
            'cfp_smtp_from_email_name' => null,
            'cfp_smtp_from_email_address' => null,
            'cfp_smtp_password' => 'smtp_password'
        );
    }
    ...
    private function get_forms_mapping()
    {
        return array(
            ...
            'cfp_welcome_email_subject' => 'form_email_subject',
            'cfp_welcome_email_message' => 'form_email_content',
...
// iv - encrypt method AES-256-CBC expects 16 bytes - else you will get a warning
    ...
    public function migration_old_crf()
    {
        ...
        global $wpdb;
        $crf_option = $wpdb->prefix . "cfp_option";
        $crf_users = $wpdb->prefix . "cfp_users";
        $crf_submissions = $wpdb->prefix . "cfp_submissions";
        $crf_db_version = 3.6;

        $crf_option = $wpdb->prefix . "cfp_option";
        $crf_fields = $wpdb->prefix . "cfp_fields";
        $crf_forms = $wpdb->prefix . "cfp_forms";
        $crf_entries = $wpdb->prefix . "cfp_entries";

        $save_db_version = floatval(get_option('cfp_db_version', '1.0'));
	...
	(9, 'userip', 'no')";
            ...
            $qry = "select `value` from $crf_option where fieldname='cfp_theme'";
            ...
            if (isset($crf_theme) && $crf_theme != "")
            {
                ...
                {
                    $wpdb->query("update $crf_option set value='classic' where fieldname='cfp_theme'");
                } else
                {
                    $wpdb->query("update $crf_option set value='default' where fieldname='cfp_theme'");
                }
            }
	...
	(20, 'mailchimp_key', '')";
            ...
            $crf_stats = $wpdb->prefix . "cfp_stats";
...
//Add column if not present.
            ...
            $submissions = get_option('cfp_migrate_submission', 'no');
            if ($submissions === 'no')
            {
                $this->crf_migrate_entries_optimized();
                error_log('submission migrated successful');
                update_option("cfp_migrate_submission", 'yes');
            }

            update_option('cfp_db_version', $crf_db_version);
        }
    }
    ...
    private function crf_entries_act_ins($dst_data)
    {
        global $wpdb;
        $wpfx = $wpdb->prefix;
        $sub_table = $wpfx . 'cfp_submissions';
    ...
    {
        global $wpdb;
        $table_name_form = $wpdb->prefix . 'cm_forms';
        $table_name_field = $wpdb->prefix . 'cm_fields';
            ...
            foreach ($this->all_forms as $form_id)
            {
                ...
                if (!in_array($form_id, $this->created))
                {
                    $wpdb->insert(
                        ...
                        'is_field_primary' => 1,
                        'field_options' => 'O:8:"stdClass":9:{s:17:"field_placeholder";s:5:"Email";s:19:"field_default_value";N;s:15:"field_css_class";s:22:"cm_form_default_fields";s:16:"field_max_length";N;s:22:"field_textarea_columns";N;s:19:"field_textarea_rows";N;s:17:"field_is_required";i:1;s:18:"field_is_read_only";i:0;s:21:"field_is_other_option";N;}'
                            ), array(
                        '%d',
                        '%s',
                        '%s',
    ...
    private function crf_assign_key_for_previous_field()
    {
        global $wpdb;
        $crf_fields = $wpdb->prefix . "cfp_fields";
        $textdomain = 'custom-user-contact-form-builder';
