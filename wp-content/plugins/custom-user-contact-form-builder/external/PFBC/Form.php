...
spl_autoload_register("CM_PFBC_Load");
if (in_array("__autoload", spl_autoload_functions()))
    spl_autoload_register("__autoload");

class CM_PFBC_Form extends CM_Base
{
    ...
    public function __construct($id = "pfbc")
    {
        ...
        if (empty($this->view))
            $this->view = new CM_View_SideBySide;

        if (empty($this->errorView))
            $this->errorView = new CM_ErrorView_Standard;
    ...
    public function addElement(CM_Element $element)
    {
        ...
        /* For ease-of-use, the form tag's encytype attribute is automatically set if the File element
          class is added. */
        if ($element instanceof CM_Element_File)
    ...
    public static function isValid($id = "pfbc", $clearValues = true)
    {
        ...
        if (!empty($form))
        {
            if ($_SERVER["REQUEST_METHOD"] == "POST")
                $data = CM_Utilities::trim_array($_POST);
            else
                $data = CM_Utilities::trim_array($_GET);
           
            if(!isset($data['cm_form_sub_id']) || $data['cm_form_sub_id'] != $id)
                return false;
    ...
    {   
        if(isset($_SESSION['cm_session'.$id]))
            self::$session=  $_SESSION['cm_session'.$id];
    ...
    protected function renderJS()
    {
        ...
        echo '<pre class="cm-pre-wrapper-for-script-tags"><script type="text/javascript">';
        $this->view->renderJS();
        foreach ($this->_elements as $element)
            $element->renderJS();
...
JS;
    ...
    protected function renderJSFiles()
    {
        ...
        if (!empty($urls))
        {
            ...
            foreach ($deps as $dep)
            {
                if(isset($urls[$dep]))
                {
                echo CM_Utilities::enqueue_external_scripts($dep, $urls[$dep]);
                unset($urls[$dep]);
                }
            }

            foreach ($urls as $handle => $url){
                echo CM_Utilities::enqueue_external_scripts($handle, $url);
            }
        }
    }
    ...
    protected function save()
    {
        ...
        $_SESSION['cm_session'.$this->form_slug]= self::$session;
    ...
    public static function _setSessionValue($id, $fields_data)
    {
        ...
        //self::$session["pfbc-".$id."-values"][$element] = CM_Utilities::trim_array($value);
    ...
    {
        $values = CM_Utilities::trim_array($values);
