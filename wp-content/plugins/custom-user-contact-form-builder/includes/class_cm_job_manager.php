...
class CM_Job_Manager {
    ...
    public static function dump() {
        ...
        /*
          ...
          {
          $result = CM_DBManager::get_submissions_for_form($job_batch[0]->form_id, $job_batch[0]->job_size, $job_batch[0]->offset, 'user_email');
          print_r($result);
          } */
        echo('</pre>');
    }
    ...
    public static function add_job($form_id, array $mail_packet, $mails_in_one_go = 20, $optional_callback = null) {
        $job_batch = get_option("cm_option_jobman_job", null);
        $inv_service = new CM_Invitations_Service;
        if ($job_batch == null)
            $job_batch = array();

        $test = CM_DBManager::count('SUBMISSIONS', array('form_id' => $form_id, 'user_email' => 'not null'));
        ...
        $job->started_on = CM_Utilities::get_current_time();
        ...
        update_option("cm_option_jobman_job", $job_batch);
        ...
        foreach ($job_batch as $key => $job) {
            ...
            if ($results != false) {
                foreach ($results as $result) {
                    ...
                    CM_Utilities::send_mail($cron_mail);
    ...
    public static function get_job_array() {
        return get_option("cm_option_jobman_job", null);
    }
