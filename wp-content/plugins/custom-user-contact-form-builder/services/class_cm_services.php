...
class CM_Services {
    ...
    public function __construct($model = null) {
        ...
        if ($this->get_setting('enable_mailchimp') == 'yes')
            $this->mailchimpService = new CM_MailChimp_Service();
    }

    public function get_fields_highest_order($form_id, $form_page_no) {
        $highest_order = 0;
        $z = CM_DBManager::get('FIELDS', array('form_id' => $form_id, 'page_no' => $form_page_no), array('%d', '%d'), 'col', 0, 2, 'field_order', 'field_order', true);
    ...
    public function get_all($model_identifier = null, $offset = 0, $limit = 9999999, $column = '*', $sort_by = '', $descending = false) {
        ...
        $results = CM_DBManager::get_all($model_identifier, $offset, $limit, $column, $sort_by, $descending);

        return $results;
    }

    public function count($model_identifier, $args, $data_specifier = '') {
        return CM_DBManager::count($model_identifier, $args, $data_specifier);
    }

    public function get_user($user_id, $field_name) {
        $user = new CM_User($user_id);
    ...
    public function get($model_identifier, $where, $data_specifier, $result_type = 'results', $offset = 0, $limit = 15, $column = '*', $sort_by = null, $descending = false) {
        return CM_DBManager::get($model_identifier, $where, $data_specifier, $result_type, $offset, $limit, $column, $sort_by, $descending);
    }
    ...
    public function duplicate($unique_ids, $model_identifier = null) {
        ...
        $model_name = CM_Utilities::get_class_name_for($model_identifier);

        $model = new $model_name;
        if (is_array($unique_ids)) {
            foreach ($unique_ids as $unique_id) {
                $model->load_from_db($unique_id, false);
                if($model instanceof CM_Forms)
    ...
    public function remove($unique_ids, $model_identifier = null, $where = null) {
        ...
        if (is_array($unique_ids)) {
            foreach ($unique_ids as $unique_id) {
                CM_DBManager::remove_row($model_identifier, $unique_id, $where);
            }
        } elseif ((int) $unique_ids) {
            CM_DBManager::remove_row($model_identifier, $unique_ids, $where);
        } else
            return false;
    }
    ...
    public function remove_submissions($unique_ids, $where = null) {
        ...
        if (is_array($unique_ids)) {
            foreach ($unique_ids as $unique_id) {
                CM_DBManager::remove_row($model_identifier, $unique_id, $where);
                CM_DBManager::delete_rows('SUBMISSION_FIELDS', array('submission_id' => $unique_id), array('%d'));
            }
        } elseif ((int) $unique_ids) {
            CM_DBManager::remove_row($model_identifier, $unique_ids, $where);
            CM_DBManager::delete_rows('SUBMISSION_FIELDS', array('submission_id' => $unique_ids), array('%d'));
        } else
            return false;
    }
    ...
    public static function import_form_first($custom_file = null, $form_id = null) {
        ...
        $form_id_next = null;
        $name = !$custom_file ? get_temp_dir() . 'Gorilla.xml': $custom_file;
        $xml = simplexml_load_file($name) or die('Incorrect XML File.');
        $cm_ser_tmp = new CM_Services;
        $xmldata = $cm_ser_tmp->xml2array($xml);
                ...
                if ($data_identifier == 'FRONT_USERS') {
                    if (isset($data_value[0])) {
                        foreach ($data_value as $form_data) {
                            $form_data_tmp = $cm_ser_tmp->xml2array($form_data);
                            $result = $cm_ser_tmp->import_front_users($form_data_tmp);
                        }
                    } else {
                        $form_data_tmp = $cm_ser_tmp->xml2array($data_value);
                        $result = $cm_ser_tmp->import_front_users($form_data_tmp);
                    }
                    ...
                    if (isset($data_value[0])) {
                        foreach ($data_value as $form_data) {
                            $form_data_tmp = $cm_ser_tmp->xml2array($form_data);
                            $result = $cm_ser_tmp->import_paypal_fields($form_data_tmp);
                        }
                    } else {
                        $form_data_tmp = $cm_ser_tmp->xml2array($data_value);
                        $result = $cm_ser_tmp->import_paypal_fields($form_data_tmp);
                    ...
                    foreach ($data_value as $form_data) {
                        if (isset($form_id)) {
                            if ($form_data->OPTIONS->form_id == $form_id) {
                                $form_data_tmp = $cm_ser_tmp->xml2array($form_data);
                                $cm_ser_tmp->import_form($form_data_tmp);
                            } else {
                            ...
                            if ($counter == 1) {
                                $form_data_tmp = $cm_ser_tmp->xml2array($form_data);
                                $result = $cm_ser_tmp->import_form($form_data_tmp);
                            } else {
    ...
    public function import_form($data_value) {
        ...
        $data_format = array();
        $form_id_old = null;
        $data_format = $cm_sr->get_format_specifer($model_identifier);
        ...
        foreach ($data_value as $data_identifier => $data_value) {
            ...
            if ($data_identifier == 'OPTIONS') {
                ...
                $form_id_new = CM_DBManager::insert_row($model_identifier, $data, $data_format);
                $cm_sr->form_id_array[$form_id_old] = $form_id_new;
            }
            if ($data_identifier == 'FIELDS') {
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $cm_sr->import_fields($form_id_new, $field_data);
                        // echo "Field ID:-",$result,"</br>"; 
                    }
                } else {
                    $result = $cm_sr->import_fields($form_id_new, $data_value);
                }
                ...
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $cm_sr->import_submissions($form_id_new, $field_data);
                ...
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $cm_sr->import_notes($form_id_new, $field_data);
                ...
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $cm_sr->import_stats($form_id_new, $field_data);
                ...
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $cm_sr->import_submission_field($form_id_new, $field_data);
                ...
                if (isset($data_value[0])) {
                    foreach ($data_value as $field_data) {
                        $result = $cm_sr->import_paypal_log($form_id_new, $field_data);
    ...
    public function import_fields($form_id_new, $fields) {
        ...
        $field_id_new = CM_DBManager::insert_row($data_identifier, $data, $data_format);
    ...
    public function import_submissions($form_id_new, $submissions) {
        ...
        $submission_id_new = CM_DBManager::insert_row($data_identifier, $all_data, $data_format);
    ...
    public function import_front_users($submissions) {
        ...
        $front_user_id_new = CM_DBManager::insert_row($data_identifier, $data, $data_format);
    ...
    public function import_notes($form_id_new, $submissions) {
        ...
        $note_id_new = CM_DBManager::insert_row($data_identifier, $data, $data_format);
        return $note_id_new;
    }
    ...
    public function import_stats($form_id_new, $submissions) {
        ...
        $stat_id_new = CM_DBManager::insert_row($data_identifier, $data, $data_format);
        return $stat_id_new;
    }
    ...
    public function import_submission_field($form_id_new, $submissions) {
        ...
        $submission_id_new = CM_DBManager::insert_row($data_identifier, $data, $data_format);
        return $submission_id_new;
    }
    ...
    public function import_paypal_log($form_id_new, $submissions) {
        ...
        $paypal_log_id_new = CM_DBManager::insert_row($data_identifier, $data, $data_format);
    ...
    public function get_mailchimp_list() {
        ...
        $lists = array(null => CM_UI_Strings::get('OPTION_SELECT_LIST'));
        if (!empty($result)) {
    ...
    public function get_all_form_fields($form_id) {
        if ((int) $form_id)
            return CM_DBManager::get_fields_by_form_id($form_id);
        else
            return false;
    }
    ...
    public function create_default_password_field($form_id) {
        ...
        $field->set(array('form_id' => $form_id,
            ...
            'field_css_class' => 'cm_form_default_fields',
            'field_is_required' => 1,
    ...
    public function remove_form_fields($form_id) {
        if (is_array($form_id))
            foreach ($form_id as $formId)
                CM_DBManager::delete_form_fields($formId);
        elseif ((int) $form_id)
            CM_DBManager::delete_form_fields($form_id);
        else
            throw new InvalidArgumentException("Invalid Form ID '$form_id'.");
    }

    public function get_setting($name) {
        $global_settings = new CM_Options;
    ...
    public function is_form_expired_core($form_id, $criterion, $submission_limit) {
        if ($criterion == "date") {
            if (CM_DBManager::is_expired_by_date($form_id))
                return true;
        }elseif ($criterion == "submissions") {
            if (CM_DBManager::is_expired_by_submissions($form_id, $submission_limit))
                return true;
        }elseif ($criterion == "both") {
            return (CM_DBManager::is_expired_by_date($form_id) || CM_DBManager::is_expired_by_submissions($form_id, $submission_limit));
        }
    }
    ...
    public function get_form_expiry_stats($form, $is_form_object = true) {
        ...
        if ($criterian == "date") {
            if (CM_DBManager::is_expired_by_date($form_id, $remaining->remaining_days)) {
            ...
            if (CM_DBManager::is_expired_by_submissions($form_id, $form_options->form_submissions_limit, $remaining->remaining_subs)) {
                $remaining->state = 'expired';
            ...
            if (CM_DBManager::is_expired_by_date($form_id, $remaining->remaining_days) || CM_DBManager::is_expired_by_submissions($form_id, $form_options->form_submissions_limit, $remaining->remaining_subs)) {
                $remaining->state = 'expired';
    ...
    public function delete_rows($model_identifier, $where, $where_format = null) {
        return CM_DBManager::delete_rows($model_identifier, $where, $where_format);
    }

    public function duplicate_form_fields($form_id, $ids) {
        if (is_array($form_id))
            foreach ($form_id as $formId) {
                $fields = CM_DBManager::get_fields_by_form_id($formId);
                foreach ($fields as $field)
                    $this->duplicate_field($field->field_id, $ids[$formId]);
            } elseif ((int) $form_id) {
            $fields = CM_DBManager::get_fields_by_form_id($form_id);
            foreach ($fields as $field)
    ...
    public function set_field_order($list) {
        CM_DBManager::set_field_order($list);
    }

    public function get_submissions_by_email($user_email, $limit = 9999999, $offset = 0, $sort_by = '', $descending = true) {
        return CM_DBManager::get_submissions_for_user($user_email, $limit, $offset, $sort_by, $descending);
    }
    ...
    public function get_payments_by_submission_id($submission_ids, $limit = 9999999, $offset = 0, $sort_by = '', $descending = false) {
        ...
        if (is_array($submission_ids))
            $fields = CM_DBManager::get_results_for_array('PAYPAL_LOGS', 'submission_id', $submission_ids);
    ...
    public function remove_form_submissions($form_id) {
        if (is_array($form_id))
            foreach ($form_id as $formId)
                CM_DBManager::delete_form_submissions($formId);
        elseif ((int) $form_id)
            CM_DBManager::delete_form_submissions($form_id);
        else
            throw new InvalidArgumentException("Invalid Form ID '$form_id'.");
    }
    ...
    public function remove_form_stats($form_id) {
        if (is_array($form_id))
            foreach ($form_id as $formId)
                CM_DBManager::delete_form_stats($formId);
        elseif ((int) $form_id)
            CM_DBManager::delete_form_stats($form_id);
        else
            throw new InvalidArgumentException("Invalid Form ID '$form_id'.");
    }
    ...
    public function remove_form_notes($form_id) {
        if (is_array($form_id))
            foreach ($form_id as $formId)
                CM_DBManager::delete_form_notes($formId);
        elseif ((int) $form_id)
            CM_DBManager::delete_form_notes($form_id);
        else
            throw new InvalidArgumentException("Invalid Form ID '$form_id'.");
    }
    ...
    public function remove_form_payment_logs($form_id) {
        if (is_array($form_id))
            foreach ($form_id as $formId)
                CM_DBManager::delete_form_payment_logs($formId);
        elseif ((int) $form_id)
            CM_DBManager::delete_form_payment_logs($form_id);
        else
            throw new InvalidArgumentException("Invalid Form ID '$form_id'.");
    }
    ...
    public function remove_submission_payment_logs($sub_id) {
        if (is_array($sub_id))
            foreach ($sub_id as $sub_id_)
                CM_DBManager::delete_rows('PAYPAL_LOGS', array('submission_id' => $sub_id_));
        elseif ((int) $sub_id)
            CM_DBManager::delete_rows('PAYPAL_LOGS', array('submission_id' => $sub_id));
        else
    ...
    public function remove_submission_notes($sub_id) {
        if (is_array($sub_id))
            foreach ($sub_id as $sub_id_)
                CM_DBManager::delete_rows('NOTES', array('submission_id' => $sub_id_));
        elseif ((int) $sub_id)
            CM_DBManager::delete_rows('NOTES', array('submission_id' => $sub_id));
        else
            throw new InvalidArgumentException("Invalid Submission ID '$sub_id'.");
    }
    ...
    public function get_submissions_to_export($form_id, $is_searched = false, $search = null) {
        ...
        if (!$is_searched && !($search instanceof stdClass)) {
            ...
            if (!$submission_ids)
                return false;
            $submissions = CM_DBManager::get_results_for_array('SUBMISSION_FIELDS', 'field_id', $field_ids);
        }
        else {
            $submission_ids = CM_DBManager::get_results_for_last_col($search->interval, $form_id, $search->id, $search->value);
            if (!$submission_ids)
                return false;
            $submissions = CM_DBManager::get_sub_fields_for_array('SUBMISSION_FIELDS', 'field_id', $field_ids, 'submission_id', $submission_ids);
        }
        ...
        foreach ($submission_ids as $s_id) {
            ...
            if ($is_payment) {
                ...
                $export_data[$s_id]['date'] = isset($payment->posted_date) ? CM_Utilities::localize_time($payment->posted_date, get_option('date_format')) : null;
            }
        }
        ...
        foreach ($submissions as $submission) {
            ...
            if (is_array($value)) {
                if (isset($value['cm_field_type']) && $value['cm_field_type'] == 'File') {
                    unset($value['cm_field_type']);
                    if (count($value) == 0)
                        $value = null;
                    else {
                        $file = array();
                ...
                }elseif (isset($value['cm_field_type']) && $value['cm_field_type'] == 'Address'){
                       unset($value['cm_field_type']);
    ...
    public function create_csv($data) {
        ...
        $csv_name = 'cm_submissions' . time() . mt_rand(10, 1000000);
    ...
    public function download_file($file, $unlink = true) { 
        ...
        if (file_exists($file)) {
            $mime_type = CM_Utilities::mime_content_type($file);
            ...
            case 'delete_page':
                if ($form_pages == null || !$page_no) {
                    ...
                    if (isset($form_pages[$page_no - 1])) {
                        CM_DBManager::remove_fields_for_page($page_no, $form_id);
 ...
 public function get_review_event()
    {
          $total_submissions=  CM_DBManager::group_by_total('SUBMISSIONS', 1,'','user_email');
    ...
    public function get_editable_fields($form_id) {
        global $wpdb;
        $table_name = $wpdb->prefix.'cm_fields';
        if ((int) $form_id) {
    ...
    public function get_latest_submission_for_user($user_email, $form_ids = array()) {
        return CM_DBManager::get_latest_submission_for_user($user_email, $form_ids);
    }
    ...
    public function get_latest_submission_from_group($submission_id){
        return CM_DBManager::get_latest_submission_from_group($submission_id);
    }
    ...
    public function get_oldest_submission_from_group($submission_id){
        return CM_DBManager::get_oldest_submission_from_group($submission_id);
    }
}
