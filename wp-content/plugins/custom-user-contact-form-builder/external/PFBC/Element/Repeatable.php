 ...
 * @author CMSHelplive
 */
class CM_Element_Repeatable extends CM_Element
{
    ...
    {
        if ($type === "start")
            echo '<div class="cm_field_type_repeatable_container" id="cm_field_type_repeatable_container_'.$this->_attributes['id'].'">';
        if ($type === "prepend")
            echo '<div class="appendable_options">';
        if ($type === "append")
            echo '<div class="cm_actions" id="cm_add_repeatable_field" onClick="cm_append_field(\'div\',\'cm_field_type_repeatable_container_'.$this->_attributes["id"].'\')"><a>' . CM_UI_Strings::get("LABEL_ADD") . '</a></div><div class="cm_actions" onClick="cm_delete_appended_field(this,\'cm_field_type_repeatable_container_'.$this->_attributes["id"].'\')"><a href="javascript:void(0)">' . CM_UI_Strings::get("LABEL_DELETE") . '</a></div></div>';
        if ($type === "close")
            echo '</div>';
    }
