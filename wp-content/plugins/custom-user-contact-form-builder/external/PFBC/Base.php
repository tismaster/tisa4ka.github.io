	...
	public function getAttributes($ignore = "") {
		...
		if(!empty($this->_attributes)) {
			...
			foreach($attributes as $attribute) {
                ...
                {
                    if(isset($this->_attributes[$attribute][$_SERVER['CM_COUNTER']]))
                        $str .= '="' . $this->filter($this->_attributes[$attribute][$_SERVER['CM_COUNTER']]) . '"';
                    if ($_SERVER['CM_COUNTER'] < (count($this->_attributes[$attribute])-1))
                        $_SERVER['CM_COUNTER'] ++;
                    else
