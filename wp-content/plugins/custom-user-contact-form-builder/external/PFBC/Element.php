<?php
abstract class CM_Element extends CM_Base {
	...
	public function isRequired() {
		if(!empty($this->validation)) {
			foreach($this->validation as $validation) {
				if($validation instanceof CM_Validation_Required)
					return true;
			}
		}
		return false;
	...
	public function isValid($value,$form_id) {
		...
		if(!empty($this->validation)) {
			...
			foreach($this->validation as $validation) {
				...
				if($validation instanceof CM_Validation_File){
	...
	public function setRequired($required) {
		if(!empty($required))
			$this->validation[] = new CM_Validation_Required;
	...
	public function setValidation($validation) {
		...
		foreach($validation as $object) {
			...
			if($object instanceof CM_Validation) {
				$this->validation[] = $object;
				if($object instanceof CM_Validation_Required)
					$this->_attributes["required"] = "";	
			}	
		}	
	}
