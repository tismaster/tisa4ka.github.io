...
/**
 * Description of class_cm_front_form_controller
 *
 * @author CMSHelplive
 */
class CM_Front_Form_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->form_factory = new CM_Form_Factory();
    }
    ...
    public function process($model, $service, $request, $params)
    {
        ...
        if(count($cm_form_diary)>0 && !isset($params['force_enable_multiform']))
            return;
        ...
        if (isset($request->req['cm_pproc'], $request->req['cm_fid'], $request->req['cm_fno'], $cm_form_diary[$form_id])
                && $request->req['cm_fid'] == $form_id && $request->req['cm_fno'] == $cm_form_diary[$form_id])
        {   $paypal_service= new CM_Paypal_Service();
            ob_start();
            $resp = $paypal_service->callback($request->req['cm_pproc'], isset($request->req['cm_pproc_id'])?$request->req['cm_pproc_id']:null);
        ...
        if ($service->is_off_limit_submission($form_id, $fopts))
                return CM_UI_Strings::get("ALERT_SUBMISSIOM_LIMIT");
        ...
        if (isset($request->req['cm_form_sub_no']) && $request->req['cm_form_sub_no'])
            $subbed_form_no = $request->req['cm_form_sub_no'];
        else
            $subbed_form_no = null;
        ...
        {         
            ...
            if ($form_options->form_should_send_email)
            {
                ...
                $parameters->email_content = $form_options->form_email_content ? $form_options->form_email_content : CM_UI_Strings::get('MAIL_REGISTRAR_DEF_BODY');
    ...
    public function test_form_access_v2(CM_Frontend_Form_Base $fe_form, $service, $request, $params)
    {
        return null;
    }
    
    //Form to edit prev submission
    public function edit_sub($model, CM_Front_Form_Service $service, $request, $params) {
        if ($service->is_ip_banned()) {
            return $this->banned_view();
        }
