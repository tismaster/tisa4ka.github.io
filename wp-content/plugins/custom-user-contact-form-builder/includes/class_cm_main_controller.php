 ...
 */
class CM_Main_Controller implements CM_Controller_Abstract
{

    private $request;
    // Attribute to load xml properties given in cm_config.xml
    private $xml;
    ...
    protected function set_xml()
    {
        ...
        $this->xml = $xml_loader->load_data($this->request->req['cm_slug']);
    ...
    public function run()
    {
        $installed_cm_version = get_site_option('cm_option_cm_version', null);

        if ((!$installed_cm_version || version_compare($installed_cm_version, CM_PLUGIN_VERSION, 'lt')) && is_admin())
        {
            update_site_option('cm_option_cm_version', CM_PLUGIN_VERSION);
            
            if(CM_SHOW_WHATSNEW_SPLASH == true)
            {
                $screen = get_current_screen();
                if ($screen->base !== 'post')
                {
                    //User either intalled fresh or updated RM, show 'Whats New' page.
                    include plugin_dir_path(plugin_dir_path(__FILE__)) . 'admin/views/template_cm_whats_new.php';
