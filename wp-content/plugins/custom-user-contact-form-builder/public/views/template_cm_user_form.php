...
<div class="cmagic">
    ...
    if ($data->fields_data)
    {
        $form = new CM_PFBC_Form('form_' . $data->form->form_id);
...
//Create stat_id field
            ...
            if (!empty($data->form->form_options->form_custom_text))
                $form->addElement(new CM_Element_HTML('<div class="cmheader">' . $data->form->form_options->form_custom_text . '</div>'));

            $form->addElement(new CM_Element_HTML("<div class='cm_input_fields_container'>"));
...
// Check if registration fields required
            if ($data->form->get_form_type() == 1 && !is_user_logged_in())
            {
                ...
                {
                    $form->addElement(new CM_Element_Radio("<b>" . $data->role_as . ":</b>", "role_as", $data->allowed_roles, array("id" => "cm_", "required" => "1")));
                }

                require_once(__DIR__ . '/template_cm_register.php');
            }
            ...
            foreach ($data->fields_data as $field_data)
            {
                if ($field_data->field_type !== 'Password' || !$data->is_auto_generate)
                {
                    ...
                    $element_class = "CM_Element_" . $field_data->field_type;
                    ...
                    if (isset($field_data->field_value))
                        ...
                        {
                            $options = array();
                            $tmp_options = CM_Utilities::trim_array(explode(',', $field_data->field_value));
                    ...
                    elseif ($field_data->field_type == 'Price')
                    {
                        ...
                        $form->addElement(new CM_Element_Hidden('cm_payment_form', $options));
                        ...
                        switch ($paypal_field->get_type())
                        {
                            case "fixed":
                                ...
                                if ($paypal_field->get_extra_options() != 'yes')
                                    $element = new CM_Element_Hidden($name, $label);
                                else
                            ...
                            case "userdef":
                                ...
                                $element = new CM_Element_Number($label, $name, $properties);
                                break;
                            ...
                            case "dropdown":
                                ...
                                $properties['id'] = 'id_cm_multisel_paypal_field';

                                if ($paypal_field->get_type() == 'multisel')
                                    $element = new CM_Element_Checkbox($label, $name, $vals, $properties);
                                else
                                {
                                    if (!isset($field_data->properties['required']))
                                        $vals = array(null => 'Select an option') + $vals;
                                    $element = new CM_Element_Select($label, $name, $vals, $properties);
                                }
                                break;
                            /*
                              case "dropdown":
                              $element = new CM_Element_Select($label, $name, $properties, $others);
                              break;
                             */
                        }
                    ...
                    } else if ($field_data->field_type === 'File')
                    {
                        ...
                        $form->addElement(new CM_Element_Hidden($name, "__RM"));
                    } else
                        $form->addElement(new $element_class($label, $name, $field_data->properties));
...
// Checking if captcha is enabled
            ...
            {
                $form->addElement(new CM_Element_Checkbox('', 'cm_subscribe_mc', array(1 => $data->form->form_options->form_opt_in_text ? : CM_UI_Strings::get('MSG_SUBSCRIBE'))));
            }
            ...
            $form->addElement(new CM_Element_Button($btn_label != "" ? $btn_label : "Submit", "submit", array("bgColor" => "#$data->submit_btn_bgcolor", "fgColor" => "#$data->submit_btn_fgcolor")));

            if ($data->expired)
                if ($data->form->form_options->form_message_after_expiry)
                    echo $data->form->form_options->form_message_after_expiry;
                else
                    echo CM_UI_Strings::get('MSG_FORM_EXPIRY');
            else
            {
                //Expiry drama
                $exp_str = '<div class="cm_expiry_stat_container">';
                if ($data->expiry_details->state !== 'perpetual')
                {
                    if ($data->expiry_details->state === 'expired')
                        $exp_str .= '<div class="cm-formcard-expired">' . 'Expired' . '</div>';
                    else
                    {
                        switch ($data->expiry_details->criteria)
                        {
                             case 'both':
                                $exp_str .= '<div class="cm-formcard-expired">' . ($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs) . ' out of ' . $data->expiry_details->sub_limit . ' filled and ' . $data->expiry_details->remaining_days . ' days to go </div>';
                            ...
                            case 'subs':
                                ...
                                if ($data->expiry_details->criteria == 'subs')
                                    $exp_str .= '<div class="cm-formcard-expired">' . ($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs) . ' out of ' . $data->expiry_details->sub_limit . ' filled' . '</div>';
                                break;

                            case 'date':
                                $exp_str .= '<div class="cm-formcard-expired">' . $data->expiry_details->remaining_days . ' days to go' . '</div>';
                                break;
                        }
                    }
