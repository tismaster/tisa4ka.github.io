<?php
class CM_Validation_File extends CM_Validation {
    protected  $name;
    protected $formId;
    ...
    public function __construct($message = "", $is_required=false) {
		if(!empty($message))
            ...
            $this->message = CM_UI_Strings::get('FORM_ERR_FILE_TYPE'); 
    ...
    function set_error($error){
        if(isset($error) && !empty($error)){
            foreach($error->errors as $error){
                CM_PFBC_Form::setError($this->formId, $error[0]);
            }
        }
    }
    ...
    public function isTypeAllowed($type){
        $valid= false;
        $allowed_types= explode('|',get_option('cm_option_allowed_file_types'));
