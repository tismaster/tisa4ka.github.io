...
class CM_Form_Factory
{
    ...
    public function create_form($form_id)
    {
        ...
        $this->backend_form = new CM_Forms;
        $this->backend_form->load_from_db($form_id);
        
        //Update form diary
        global $cm_form_diary;
        if(isset($cm_form_diary[$form_id]))
            $cm_form_diary[$form_id]++;
        else
            $cm_form_diary[$form_id] = 1;
        ...
        if($db_fields)
        {
            foreach($db_fields as $db_field)
            {
                ...
                $field_factory= new CM_Field_Factory($db_field,$opts);
            ...
            {
                case CM_REG_FORM:                    
                    $this->frontend_form = new CM_Frontend_Form_Reg($this->backend_form);
                ...
                default:
                    //$this->frontend_form = new CM_Frontend_Form_Multipage($this->backend_form);                    
                    $this->frontend_form = new CM_Frontend_Form_Contact($this->backend_form);
                    $this->frontend_form->set_primary_field_index($primary_field_req_names);
                    break;
            }              
            
            $this->frontend_form->add_fields_array($fields);      
            $this->frontend_form->set_form_number($cm_form_diary[$form_id]);
