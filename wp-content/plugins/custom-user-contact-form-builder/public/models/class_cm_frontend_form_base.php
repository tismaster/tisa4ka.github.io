...
abstract class CM_Frontend_Form_Base
{
    ...
    public function __construct(CM_Forms $be_form, $ignore_expiration=false)
    {
        ...
        $this->service = new CM_Front_Form_Service;
    }
    ...
    public function add_field(CM_Frontend_Field_Base $fe_field)
    {
    ...
    protected function pre_render()
    {
        ...
        if($this->form_options->btn_hover_color)
            echo '.cmagic #form_' . $this->form_id . "_" . $this->form_number .' .buttonarea input[type="button"]:hover{ background-color:'.$this->form_options->btn_hover_color.$important.';}';
        if($this->form_options->field_bg_focus_color || $this->form_options->text_focus_color){
            echo '.cmagic #form_' . $this->form_id . "_" . $this->form_number .' .cmrow input:focus,.cmagic #form_'.$this->form_id.'_'.$this->form_number.' .cmrow select:focus,.cmagic #form_'.$this->form_id.'_'.$this->form_number.' .cmrow textarea:focus{';
        ...
        if (!$this->is_expired())
        {
            ...
            if ($expiry_details->state !== 'perpetual' && $this->service->get_setting('display_progress_bar') == 'yes')
            {
                ...
                else
                {
                    ...
                    {
                        case 'both':
                            $exp_str .= '<div class="cm-formcard-expired"><span class="cm_sandclock"></span>' . ($expiry_details->sub_limit - $expiry_details->remaining_subs) . ' out of ' . $expiry_details->sub_limit . ' filled and ' . $expiry_details->remaining_days . ' days to go </div>';
                            break;
                        case 'subs':
                            ...
                            $rem = ($rem * 100) / $total;
                            $done = 100 - $rem;
                            $exp_str .= '<div class="cm-formcard-expired"><span class="cm_sandclock"></span>' . ($expiry_details->sub_limit - $expiry_details->remaining_subs) . ' out of ' . $expiry_details->sub_limit . ' filled' . '</div>';
                            break;

                        case 'date':
                            $exp_str .= '<div class="cm-formcard-expired"><span class="cm_sandclock"></span>' . $expiry_details->remaining_days . ' days to go' . '</div>';
                            break;
                    }
                }
    ...
    protected function prepare_button_for_render($form)
    {
        ...
        $form->addElement(new CM_Element_Button($btn_label != "" ? $btn_label : "Submit", "submit", array("style" => isset($this->form_options->style_btnfield)?$this->form_options->style_btnfield:null)));
    }
